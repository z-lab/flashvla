...
from flashvla.policies.lingbot.transformers_compat import ROPE_INIT_FUNCTIONS, SlidingWindowCache
...
class Qwen2_5_VLVisionSdpaAttention(nn.Module):
    """Memory-efficient block-diagonal vision attention for fixed-size images.

    The eager implementation concatenates every image/window into one long
    sequence and materializes a masked ``[H, total_L, total_L]`` score tensor.
    RoboTwin resizes every camera to the same grid, so ``cu_seqlens`` describes
    equal-length independent blocks. Treating those blocks as the SDPA batch
    dimension is mathematically equivalent and makes memory scale linearly
    with the training batch size instead of quadratically.
    """
    ...
    def forward(
        ...
        if num_sequences <= 0 or seq_length % num_sequences != 0:
            raise ValueError(
                "LingBot vision SDPA requires non-empty equal-length cu_seqlens; "
                f"got total_length={seq_length}, num_sequences={num_sequences}"
            )
        sequence_length = seq_length // num_sequences

        def as_sdpa_batch(tensor: torch.Tensor) -> torch.Tensor:
            return (
                tensor.reshape(num_sequences, sequence_length, self.num_heads, self.head_dim)
                .permute(0, 2, 1, 3)
                .contiguous()
            )
...
class Qwen2_5_VisionTransformerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    ...
    def preprcess_grid_thw(self, grid_thw: torch.Tensor):
        """Precompute shape-only vision metadata for repeated fixed-grid calls."""
    ...
    def forward(
        ...
        if (
            ...
            (
                ...
                cu_window_seqlens,
                cu_seqlens,
            ) = self.preprcess_grid_thw(grid_thw)
...
def repeat_kv(hidden_states: torch.Tensor, n_rep: int) -> torch.Tensor:
    """Expand grouped K/V heads to the full attention-head count."""
...
class Qwen2_5_VLFlashAttention2(Qwen2_5_VLAttention):
    ...
    def forward(
        ...
        # Layer norms may cast hidden states to float32 for training stability,
        # so cast them back before attention when needed.
...
QWEN2_5_VL_ATTENTION_CLASSES = {
    ...
    "sdpa": Qwen2_5_VLAttention,  # fallback to eager for sdpa
