...
try:
    ...
    # Server-side (flashvla env) doesn't have sapien, but CONFIGS_PATH is
    # unused here anyway — camera/embodiment configs are only needed by the
    # client.  Compute a fallback so the name exists if anything references it.
    CONFIGS_PATH = os.path.join(os.path.abspath("."), "task_config/")
