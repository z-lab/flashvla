...
class ModelClient:
    def __init__(self, host='localhost', port=9999, timeout=300):
        # Default 300s socket timeout — the FIRST get_action call may trigger
        # server-side torch.compile + CUDA-graph capture (~30-90s under
        # max-autotune), and 30s isn't enough. Steady-state calls return in
        # milliseconds so the long timeout is essentially never hit again.
        self.host = host
        self.port = port
...
def main(usr_args):
    ...
    st_seed, suc_num, agg_metrics, per_ep_metrics = eval_policy(
        task_name,
        TASK_ENV,
        args,
        model,
    ...
    with open(file_path, "w") as file:
        ...
        # Efficiency/latency summary (per-step roundtrip = TCP + server inference).
        # For pi05_flashvla each policy call ≈ 1 env step; for chunk-replay
        # policies (vanilla pi05 with pi0_step=50) it covers pi0_step steps.
        if agg_metrics.get("n_episodes", 0) > 0:
            file.write("\n\n=== Efficiency ===\n")
            for k in (
                "n_episodes", "n_success", "pc_success",
                "steps_per_ep_mean", "steps_per_ep_max",
    ...
    # Machine-readable per-episode + aggregated metrics
    metrics_path = os.path.join(save_dir, "_metrics.json")
    with open(metrics_path, "w") as f:
        json.dump({"aggregated": agg_metrics, "per_episode": per_ep_metrics}, f, indent=2)
...
def eval_policy(task_name,
    ...
    # Per-episode metrics: steps (TASK_ENV.take_action_cnt at termination),
    # wall-clock (rollout loop only — excludes setup_demo / expert replay /
    # video ffmpeg startup), and per-policy-call latency. For pi05_flashvla
    # each `eval_func` call ≈ 1 env step → these latencies are per-env-step
    # roundtrip (TCP + server inference). For chunk-replay policies (e.g.
    ...
    while succ_seed < test_num:
        ...
        while TASK_ENV.take_action_cnt < TASK_ENV.step_lim:
            ...
            if TASK_ENV.eval_success:
                succ = True
                break
        ep_wall_s = time.perf_counter() - ep_wall_t0
        ep_steps = int(TASK_ENV.take_action_cnt)
        per_ep.append({
            "seed": int(now_seed),
            "success": bool(succ),
            "steps": ep_steps,
    ...
    # Aggregated metrics across all evaluated episodes for this task.
    n = len(per_ep)
    if n > 0:
        ...
        agg = {
            ...
            "pc_success": 100.0 * TASK_ENV.suc / n,
            "steps_per_ep_mean": float(steps_arr.mean()),
            "steps_per_ep_max":  float(steps_arr.max()),
            "wall_per_ep_s_mean": float(wall_arr.mean()),
            "total_wall_s": float(wall_arr.sum()),
            "time_per_step_ms_mean": float(tps_arr.mean()),
            "call_lat_ms_mean": float(np.mean(all_call_lats_ms)) if all_call_lats_ms else 0.0,
    ...
    return now_seed, TASK_ENV.suc, agg, per_ep
